...
const ImageGenerateModal = ({ onClose, title, onSuccess, ai }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const base64ImageBytesArray = response.generatedImages.map(
                img => `data:image/jpeg;base64,${img.image.imageBytes}`
            );
            setImages(base64ImageBytesArray);
            onSuccess();
        } catch (e) {
            console.error(e);
        ...
        <Modal onClose={onClose} title={title}>
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label htmlFor="num-images" className="block text-sm font-medium text-slate-700 mb-1">
                            Số lượng ảnh
                        </label>
                        <select
                            id="num-images"
                            ...
                            className="w-full p-2 border rounded-md focus:ring-2 focus:ring-yellow-400 bg-white"
                        >
                        ...
                        <div className="flex flex-wrap gap-2 items-center h-full">
                           {aspectRatios.map(ratio => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 text-sm font-semibold rounded-md transition ${aspectRatio === ratio ? 'bg-yellow-500 text-white shadow' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}
                                >
                                    {ratio}
                                </button>
                           ))}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {images.map((imgSrc, index) => (
                                <div key={index} className="space-y-2">
                                    <img src={imgSrc} alt={`Generated image ${index + 1}`} className="rounded-md w-full" />
                                    <a href={imgSrc} download={`generated-image-${index + 1}.jpg`} className="inline-block bg-slate-600 text-white font-bold py-2 px-4 rounded-md hover:bg-slate-700 transition w-full text-center">
                                        Tải xuống
                                    </a>
                                </div>
                            ))}
